<div class="flex w-full flex-col items-center bg-white" x-data>
	<div class="flex w-full justify-center bg-white py-6 shadow-sm shadow-gray-200">
		<div class="max-w-8xl w-full px-4 sm:px-6 lg:mx-auto lg:px-8">
			<div class="sm:flex sm:items-center sm:justify-between">
				...
				<div class="mt-5 flex justify-center sm:mt-0">
					<a href="{{ route('profile.show') }}" class="flex items-center justify-center rounded-md bg-white px-3 py-2 text-sm font-semibold text-gray-900 shadow-xs ring-1 ring-inset ring-gray-300 hover:bg-gray-50">
                        View profile
                    </a>
				</div>
			</div>
		...
		<div class="max-w-4xl flex w-full flex-col px-4 sm:px-6 lg:mx-auto lg:px-8">
            <x-livewire-form action="save">
                ...
                <x-slot name="content">
                    <div class="w-full flex flex-col">
                        ...
                        <div class="w-full flex flex-col space-y-4">
                            <x-apex::input.text label="Community Name" wire:model="form.name" class="max-w-xs" autocorrect="off" autocapitalize="none" />

                            <x-apex::input.textarea label="Description" wire:model="form.description" rows="4" placeholder="Enter a short description for you new community..." />
                            ...
                            <div class="w-full flex mt-2">
                                <x-apex::input.checkbox wire:model="form.track_member_balances" label="Track member balances and transactions" />
                            </div>
                        </div>
                    </div>
                </x-slot>
                ...
                <x-slot name="actions" class="w-full md:w-auto">
                    <x-apex::button variant="primary" class="w-full md:w-auto" type="submit">{{ __('Create Community') }}</x-apex::button>
                    <x-apex::button class="w-full md:w-auto" wire:click="$dispatch('closeModal')">{{ __('Cancel') }}</x-apex::button>
                </x-slot>
            </x-livewire-form>
		</div>
    </div>
